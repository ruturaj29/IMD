...
//						}
						...
						if ( (rtc.min % logInterval == 0) && (rtc.sec == 0) ) {
								//Slog();
								memset(_buffer, 0, 512);	/* Clear the Buffer */
								sprintf(_buffer,"#;%02d/%02d/%d;%02d:%02d:00,%04s;%04s,0",
								(uint16_t)rtc.date,(uint16_t)rtc.month,(uint16_t)rtc.year,
								(uint16_t)rtc.hour,(uint16_t)rtc.min,_Temperature,_Humidity);
									Createlog(_buffer, "ATRH_DATA.txt");
								 DELAY_ms(1000);
								}
