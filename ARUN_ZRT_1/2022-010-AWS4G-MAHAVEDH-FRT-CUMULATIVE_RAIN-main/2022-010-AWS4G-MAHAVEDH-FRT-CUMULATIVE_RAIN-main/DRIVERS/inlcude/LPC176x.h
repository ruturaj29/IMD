...
#define	PCONP_LPC176xx		(*(volatile uint32_t*)0x400FC0C4)
...
#define __set_PINSEL(p,b,v)		PINSEL[(p) * 2 + (b) / 16] = (PINSEL[(p) * 2 + (b) / 16] & ~(3U << ((b) * 2 % 32))) | (v << ((b) * 2 % 32))
...
#define	__set_PCONP(p,v)	PCONP_LPC176xx = (PCONP_LPC176xx & ~(1 << (p))) | (1 << (p))
//#define	PCTIM0	1
//#define	PCTIM1	2
//#define	PCUART0	3
//#define	PCUART1	4
