...
void LCD_Init(uint8_t v_lcdNoOfLines_u8, uint8_t v_MaxCharsPerLine_u8)
{
    ...
    LCD_CmdWrite(CMD_DISPLAY_ON_CURSOR_OFF);     // Display ON cursor ON
...
void LCD_ScrollMessage(uint8_t v_lineNumber_u8, char *ptr_msgPointer_u8)
{
    ...
    LCD_CmdWrite(CMD_DISPLAY_ON_CURSOR_OFF);              // Finally enable the Cursor
}
#endif
...
void LCD_DisplayFloatNumber(float v_floatNum_f32)
{
    ...
    v_decNumber_u32 = v_floatNum_f32 * 100;
    LCD_DisplayNumber(C_DECIMAL_U8,v_decNumber_u32,C_DisplayDefaultDigits_U8);
}
#endif
...
void lcd16x2_create_custom_char(uint8_t location, uint8_t* data_bytes)
{
	...
	// Set CGRAM address
	LCD_CmdWrite(LCD16X2_SET_CGRAM_ADDRESS | (location << 3));
	
	// Write 8 bytes custom char pattern
	for (i = 0; i < 8; i++) 
	{		
		lcd_DataWrite(data_bytes[i]);
	}
}
